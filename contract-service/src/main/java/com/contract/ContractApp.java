...
public class ContractApp {
	 ...
	 * web3j.transactionObservable().subscribe(tx -> { if (tx.getTo() != null &&
	 * tx.getTo().equals(service.getOwnerAccount())) {
	 * LOGGER.info("New tx: id={}, block={}, from={}, to={}, value={}",
	 * tx.getHash(), tx.getBlockHash(), tx.getFrom(), tx.getTo(),
	 * tx.getValue().intValue());
	 * service.processContracts(tx.getValue().longValue()); } else {
